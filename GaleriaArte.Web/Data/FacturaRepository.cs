    ...
    public class FacturaRepository
    {
            ...
            int idUsuario)
        {
            ...
            await using SqlCommand command =
                new SqlCommand("sp_Facturas_Crear", connection);
        ...
        public async Task<List<Obra>> ObtenerObrasDisponiblesAsync()
        {
            ...
            while (await reader.ReadAsync())
            {
                facturas.Add(new Factura
                {
                    ...
                    IdEstadoFactura = reader.GetInt32(
                        reader.GetOrdinal("IdEstadoFactura")
                    ),

                    FechaFactura = Convert.ToDateTime(
                        reader["FechaFactura"]
                    ),
        ...
        public async Task<Factura?> ObtenerFacturaPorIdAsync(
            int idFactura)
        {
            await using SqlConnection connection =
                DatabaseConnection.CreateConnection();
                ...
                while (await reader.ReadAsync())
                {
                    factura.Detalles.Add(
                        new DetalleFactura
                        {
                            ...
                            CodigoObra =
                                reader["CodigoObra"]?.ToString()
                                ?? string.Empty,

                            NombreObra =
                                reader["NombreObra"]?.ToString()
                                ?? string.Empty
                        }
                    );
                }
